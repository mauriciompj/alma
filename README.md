...
- (Optional) An [ElevenLabs](https://elevenlabs.io) API key (for voice synthesis)
- Node.js 18+
...
- **Database**: Neon PostgreSQL (serverless) with configurable full-text search (`SEARCH_LANGUAGE` — supports any language)
...
- **Chunks**: Your memories are stored as searchable text chunks in PostgreSQL with `tsvector` indexing. The search language is configurable via `SEARCH_LANGUAGE` env var (`simple` for universal, `portuguese`, `english`, `spanish`, etc.)
...
- [x] Voice synthesis via ElevenLabs TTS (hear ALMA speak)
- [x] Visual memory browser (BD Revisor — browse, filter, edit chunks)
- [x] SQL import system for structured memory batches
